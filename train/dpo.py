...
class DPODataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return chosen_ids, rejected_ids, len(prompt)


def log_probs(model, ids: torch.Tensor, prompt_len: int, ctx) -> torch.Tensor:
    """Compute sum of log-probs over the response tokens only."""
    with ctx:
        logits, _ = model(ids)
    log_p = F.log_softmax(logits, dim=-1)
    # Gather log-probs of actual tokens
    tokens = ids[:, 1:].unsqueeze(-1)          # (B, T-1, 1)
    lp = log_p[:, :-1].gather(-1, tokens).squeeze(-1)  # (B, T-1)
    # Mask prompt tokens
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  IndicLLM DPO Alignment")
    ...
    dataset = DPODataset(args.data, tokenizer, block_size)
    ...
    for epoch in range(100):
        for chosen, rejected, prompt_len in loader:
            ...
            loss = dpo_loss(policy_chosen_lp, policy_rejected_lp,
                            ref_chosen_lp,    ref_rejected_lp, args.beta)
            ...
            if step % 500 == 0 and step > 0:
                torch.save({"model": policy.state_dict(), "step": step, "config": cfg},
                           args.output / "ckpt.pt")
            step += 1
        if step >= args.max_iters:
            break

    torch.save({"model": policy.state_dict(), "config": cfg}, args.output / "final.pt")
