...
# ── Quality filter ────────────────────────────────────────────
def is_quality_indic(text: str, lang: str, min_chars: int = 50,
                     min_script_ratio: float = 0.3) -> bool:
...
# ── Tokenizer ─────────────────────────────────────────────────
def get_or_train_tokenizer(texts_sample: list[str], out_dir: Path,
...
def encode_and_write(texts: list[str], tokenizer, out_path: Path,
                     use_gpt2: bool = False) -> int:
...
def main():
    ...
    parser.add_argument("--max-docs",    type=int, default=10000,
                        help="Max docs per language (default: 10000)")
    ...
    print(f"\n{'='*60}")
    print(f"  IndicLLM-Bharat — Indic Data Pipeline")
    ...
    train_tokens = encode_and_write(
        train_texts, tokenizer, out_dir / "train.bin",
        use_gpt2=args.use_gpt2_tok
    )
    val_tokens = encode_and_write(
        val_texts, tokenizer, out_dir / "val.bin",
        use_gpt2=args.use_gpt2_tok
    )
...
"""
    ...
    print(f"\n{'='*60}")
    print(f"  ✅ Indic data pipeline complete!")
