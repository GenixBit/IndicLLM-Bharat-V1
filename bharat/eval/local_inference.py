...
class LocalInferenceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.max_new_tokens, bool) or not isinstance(
            ...
            raise TypeError(
                "max_new_tokens must be an integer, " f"got {type(self.max_new_tokens).__name__}"
            )
...
class LocalCausalLMAdapter:
    ...
    def predict(self, example: EvalExample) -> str:
        ...
        if generated.dim() != 2 or generated.shape[0] != 1:
            raise ValueError(
                "Generated IDs must have shape (1, sequence), " f"got {tuple(generated.shape)}"
            )
...
def _as_int_sequence(values: Sequence[object]) -> list[int]:
    ...
    for value in values:
        if not isinstance(value, int):
            raise TypeError("generated token IDs must be integers, " f"got {type(value).__name__}")
        ids.append(value)
    return ids
