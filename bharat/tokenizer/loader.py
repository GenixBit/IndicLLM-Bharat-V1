    ...
    def fingerprint(self) -> str:
        ...
        if tok is not None:
            data = {
                ...
                "pre_tokenizer": str(tok.pre_tokenizer.__class__.__name__) if tok.pre_tokenizer else None,
                "special_tokens": {t.content: tid for tid, t in tok.get_added_tokens_decoder().items()},
...
class _SentencePieceWrapper(BharatTokenizer):
    ...
    def fingerprint(self) -> str:
        ...
        data = {
            ...
            "vocab": {tok.id_to_token(i): i for i in range(tok.get_vocab_size()) if tok.id_to_token(i) is not None},
            "normalizer": str(tok.normalizer.__class__.__name__) if hasattr(tok, "normalizer") and tok.normalizer else None,
            "pre_tokenizer": str(tok.pre_tokenizer.__class__.__name__) if hasattr(tok, "pre_tokenizer") and tok.pre_tokenizer else None,
            "model_type": str(tok.model.__class__.__name__) if hasattr(tok, "model") else None,
            "decoder": str(tok.decoder.__class__.__name__) if hasattr(tok, "decoder") and tok.decoder else None,
        }
        return hashlib.sha256(json.dumps(data, sort_keys=True, default=str).encode()).hexdigest()
...
def _detect_model_type(tok: HFTokenizersTokenizer) -> str:
    ...
    if "sentencepiece" in model_name or "bpe" in model_name:
        if hasattr(model, "piece_token") or model_name.startswith("sentencepiece"):
            return "sentencepiece"
